...
function sasoDoActions(data) {
    ...
    if (typeof data.exclude_order_discount_details == "boolean") {
        window.saso_config.exclude_order_discount_details = data.exclude_order_discount_details
    }
...
function sasoHideNotification(hash) {
    ...
    window.sasoDocCookies.setItem("saso_notifications_closed", cooka.join(","), window.saso_config.hide_closed_notifications_for, "/")
}
...
function sasoShowNotifications(notifications) {
    if (window.saso_config.notifications.position == "top") {
        ...
        checks.map(function(sel) {
            if (jQuery("#saso-notifications").length == 0 && jQuery(sel + ":not(.saso-ignore)").length == 1) {
    ...
    var style = "background-color: " + window.saso_config.notifications.background_color + "; color: " + window.saso_config.notifications.text_color + ";";
    var style_container = style + "opacity: " + window.saso_config.notifications.opacity + "; ";
    if (jQuery("#saso-notifications").length) {} else {
        style_container += "position: fixed; " + window.saso_config.notifications.position + ": 0; "
    }
    var html = '<div class="saso-notification" style="' + style_container + '"><div class="saso-notification-x"><a href="#" style="" onclick="sasoHideNotification(\'' + hash + "'); return false;\">X</a></div>";
...
function sasoAddCartItems(items, callback) {
    if (items.length) {
        ...
        jQuery.ajax({
            ...
            error: function(res) {
                if (typeof res == "object" && typeof res.responseJSON == "object" && typeof res.responseJSON.description == "string") {
...
function sasoPrepareCrossSell(action) {
    if (typeof window.saso_config.show_upsell_only_in == "string" && window.saso_config.show_upsell_only_in.length) {
        if (window.saso_config.show_upsell_only_in.indexOf(window.saso.page_type) == -1) {
            return
        }
    }
    if (typeof window.saso_config.show_upsell_only_width_larger == "number") {
        if (Math.max(document.documentElement.clientWidth, window.innerWidth || 0) < window.saso_config.show_upsell_only_width_larger) {
    ...
    action.products.map(function(p) {
        ...
        defs.push(jQuery.getJSON("/products/" + p.handle + ".json", function(data) {
            var product = data.product;
            product = sasoCleanBuyxHandleJson(product);
            for (var i = 0; i < product.variants.length; i++) {
                if (typeof product.variants[i].compare_at_price == "string") {
                    product.variants[i].compare_at_price = parseFloat(product.variants[i].compare_at_price) * 100
                }
                if (typeof product.variants[i].price == "string") {
                    product.variants[i].price = parseFloat(product.variants[i].price) * 100
                }
                if (typeof action.price_discount == "object" && action.price_discount && typeof action.price_discount.amount == "number") {
                    product.variants[i].compare_at_price = product.variants[i].price;
                    switch (action.price_discount.type) {
                        case "flat":
                            product.variants[i].price = action.price_discount.amount * 100;
                            break;
                        case "subtract":
                            product.variants[i].price -= action.price_discount.amount * 100;
                            ...
                            break;
                        case "percent":
                            product.variants[i].price = product.variants[i].price * (100 - action.price_discount.amount) / 100;
                            break
                    }
                }
                product.variants[i].compare_at_price_n = product.variants[i].compare_at_price;
                product.variants[i].price_n = product.variants[i].price;
                if (product.variants[i].compare_at_price && product.variants[i].compare_at_price > product.variants[i].price) {
                    product.variants[i].compare_at_price = sasoShopifyformatMoney(product.variants[i].compare_at_price, window.saso.money_format)
                } else {
                    product.variants[i].compare_at_price = ""
                }
                product.variants[i].price = sasoShopifyformatMoney(product.variants[i].price, window.saso.money_format)
            }
            ...
            if (product.variants.length == 1) {
                ...
                product.variants.map(function(v) {
                    if (typeof v.inventory_management == "string" && v.inventory_management == "shopify") {
                        if (typeof v.inventory_policy == "string" && v.inventory_policy == "deny" && typeof v.inventory_quantity == "number" && v.inventory_quantity <= 0) {
                            return
                        }
                    }
                    var img = "";
                    if (typeof v.image_id == "number" && v.image_id != null) {
                        ...
                        if (typeof product.image == "object" && product.image && typeof product.image.src == "string") {
                    ...
                    variants_html += "<option value='" + v.id + "' data-img='" + img + "' data-price='" + v.price_n + "' data-compare-at-price='" + v.compare_at_price_n + "'>";
                    variants_html += sasoEscapeHtml(v.title) + "</option>"
                });
                product.variants_select = "<select class='saso-variants'>";
                if (typeof window.saso_config.upsell_variant_choose_option == "string" && window.saso_config.upsell_variant_choose_option.length) {
                    product.variants_select += "<option value='0'>" + sasoEscapeHtml(window.saso_config.upsell_variant_choose_option) + "</option>"
                }
            ...
            if (typeof product.image == "object" && product.image && typeof product.image.src == "string") {
                ...
                if (n >= 0) {
                    var s = product.image.src.substring(0, n) + "_medium." + product.image.src.substring(n + 1);
                    product.image.src = s
                }
            }
            product.title = sasoTrimLength(product.title, window.saso_config.product_title_max_length);
    ...
    jQuery.when.apply(jQuery, defs).done(function() {
        ...
        if (typeof action.from_offer_type == "string" && action.from_offer_type == "Free Gift") {
            window.saso.ignore_previous_free_gifts = false
        }
        if (products.length == 1 && products[0].variants.length == 1 && products[0].variants[0].price_n == 0) {
    ...
    setTimeout(function() {
        ...
        window.sasoc.magnificPopup.open({
            ...
            callbacks: {
                afterClose: function() {
                    sasoShowCrossSellClosed();
                    if (window.saso.page_type == "cart" && typeof window.saso.upsell_added == "boolean" && window.saso.upsell_added) {
                        location.reload()
                    }
                }
            }
        ...
        setTimeout(function() {
            if (jQuery(".saso-products-container table").length) {
                jQuery(".mfp-content").css("max-width", jQuery(".saso-products-container table").width() + 100)
            }
        }, 100);
        setTimeout(function() {
            sasoShowCrossSellClosed()
...
function sasoShowCrossSellClosed() {
    ...
    if (cooka.indexOf(a.id) == -1) {
        if (typeof window.saso.action_crosssell_popup_action.manually_closed == "boolean" && window.saso.action_crosssell_popup_action.manually_closed) {
            cooka.push(a.id)
        } else if (typeof a.price_discount == "object" && a.price_discount) {} else if (typeof a.price_bundle == "object" && a.price_bundle) {} else {
            cooka.push(a.id)
        }
    }
    window.sasoDocCookies.setItem("saso_shown_upsells", cooka.join(","), window.saso_config.hide_shown_upsells_for, "/")
}
...
function sasoPrepareBundle(action) {
    ...
    jQuery.when.apply(jQuery, defs).done(function() {
        ...
        if (typeof window.saso_config.translate_percent_off == "string") {
            action.price.title = action.price.title.replace("% Off", window.saso_config.translate_percent_off)
        }
...
function sasoBundleAddToCart(ev) {
    ...
    jQuery(".saso-product-container").each(function() {
        ...
        items_to_add_count++;
        var $variants = jQuery(".saso-product-container[data-product-id='" + product_id + "']").find("select.saso-variants");
...
function sasoShowCartDiscounts(discounts) {
    ...
    discounts.cart.items.forEach(function(item) {
        if (item.discounted_price < item.original_price) {
            jQuery(".saso-cart-item-discount-notes[data-key='" + item.key + "']").html(item.discount_notes.join("<br>"));
            jQuery(".saso-cart-item-price[data-key='" + item.key + "']").html("<span class='original_price'>" + item.original_price_format + "</span>" + "<span class='discounted_price'>" + item.discounted_price_format + "</span>");
            jQuery(".saso-cart-item-line-price[data-key='" + item.key + "']").html("<span class='original_price'>" + item.original_line_price_format + "</span>" + "<span class='discounted_price'>" + item.discounted_line_price_format + "</span>")
        }
        if (item.upsell_notes.length > 0) {
            ...
            jQuery(".saso-cart-item-upsell-notes[data-key='" + item.key + "']").html(un)
        }
    });
    if (typeof discounts.discounted_price_html != "string") {
        return
    }
    if (typeof discounts.allow_use_discount_code_cart == "boolean" && discounts.allow_use_discount_code_cart) {
        var html_d = '<div class="saso-use-discount-code-cart-container"><input type="text" class="saso-use-discount-code-cart-code" placeholder="Discount code"> ' + '<button type="button" class="saso-use-discount-code-cart-apply btn btn--secondary">Apply</button></div>';
    ...
    if (typeof discounts.allow_use_discount_code_instead == "boolean" && discounts.allow_use_discount_code_instead) {
        var html_d = '<div><label style="font-weight: normal; cursor: pointer;"><input type="checkbox" id="saso-use-discount-code-instead-check"> I will be using a coupon instead</label></div>';
    ...
    if (window.sasoDocCookies.getItem("saso_discount_code")) {
        if (typeof discounts.discount_code_cart_valid != "boolean") {
            ...
            jQuery(".saso-use-discount-code-cart-code").val(window.sasoDocCookies.getItem("saso_discount_code"));
            jQuery("#saso-use-discount-code-cart-code").val(window.sasoDocCookies.getItem("saso_discount_code"))
        }
    }
    ...
    sasoCurrency();
    var checkout_selectors = ["input[name='checkout']:not(.bold_clone)", "button[name='checkout']", "[href$='checkout']", "input[name='goto_pp']", "button[name='goto_pp']", "input[name='goto_gc']", "button[name='goto_gc']", ".additional-checkout-button", ".google-wallet-button-holder", ".amazon-payments-pay-button"];
        ...
        if (typeof els == "object" && els) {
            for (var i = 0; i < els.length; i++) {
                ...
                el.addEventListener("click", function(ev) {
                    ...
                    ev.preventDefault();
                    var elsbold = document.querySelectorAll("input[name='checkout'].bold_clone");
...
function sasoCheckout() {
    if (jQuery("input[type='checkbox'].saso_agree").length > 0 && jQuery("input[type='checkbox'].saso_agree:checked").length != jQuery("input[type='checkbox'].saso_agree").length) {
        return
    }
    if (typeof window.saso_config.force_required_fields == "boolean" && window.saso_config.force_required_fields) {
        ...
        jQuery("[name^='attributes'][required]:visible").each(function() {
            if (jQuery(this).val() == "" || jQuery(this).val() == null) {
                if (jQuery(this).attr("placeholder")) {
                    ...
                    required_field_names.push(jQuery(this).attr("name").replace("attributes[", "").replace("]", ""))
                }
                required_pass = false
            }
        });
        ...
        if (!required_pass) {
            alert("Please fill in required fields: " + required_field_names.join(", "));
            return
        }
    }
    var note_attributes = [];
    ...
    if (typeof window.saso.discounts == "object" && typeof window.saso.discounts.cart == "object" && typeof window.saso.discounts.cart.summary_note == "string") {
        note_attributes.push({
    ...
    for (var i = 0; i < window.saso.cart.items.length; i++) {
        ...
        if (el.length == 1) {
            if (el[0].value != window.saso.cart.items[i].quantity) {
                console.log("el[0].value != window.saso.cart.items[i].quantity", el[0].value);
                window.saso.cart.items[i].quantity_before_saso_update = window.saso.cart.items[i].quantity;
    ...
    if (note_attributes.length) {
        note_attributes.map(function(a) {
            invoice_url_params.push("attributes" + encodeURIComponent("[" + a.name + "]") + "=" + encodeURIComponent(a.value))
        })
    }
    if (typeof window.saso_config.checkout_start != "number") {
        ...
        var invoice_url = "/checkout?discount=&clear_discount=1&discount_issue=checkout_clicked_after_3sec_delay";
    ...
    if (window.saso_config.offers_method == "discount_code") {
        jQuery.ajax({
            ...
            dataType: "json",
            type: "POST",
            url: "https://" + sasoGetHost() + "/api/v2/page-actions?checkout=discount_code",
            ...
            success: function(res) {
                ...
                if (typeof res.discount_code == "string") {
                    ...
                    window.sasoDocCookies.setItem("saso_generated_discount_code", res.discount_code, Infinity, "/")
                }
                if (invoice_url_params.length) {
...
function sasoUseDiscountCodeInsteadChange() {
    ...
    if (did.checked) {
        ...
        window.sasoDocCookies.setItem("saso_use_discount_code_instead", true, Infinity, "/")
    } else {
    ...
    if (typeof window.saso.discounts != "object" || typeof window.saso.discounts.cart != "object" || typeof window.saso.discounts.cart.items != "object") {
        return
    }
...
function sasoShowVolumeDiscountTiers(data) {
    ...
    for (var i = 0; i < tiers_copy.length; i++) {
        ...
        if (typeof window.saso_config.translate_percent_off == "string") {
            price.title = price.title.replace("% Off", window.saso_config.translate_percent_off)
        }
        if (price.type == "subtract") {
            price.type = "flat";
            price.amount = window.saso.product.price - price.amount * 100;
            price.title = '<span class="saso-price">' + sasoShopifyformatMoney(price.amount, window.saso.money_format) + "</span>"
        }
        var pf = JSON.parse(JSON.stringify(price));
        if (price.type == "percent") {
            price_type = "percent";
            pf.amount = window.saso.product.price * (100 - data.tiers[i].price.amount) / 100;
            pf.title = '<span class="saso-price">' + sasoShopifyformatMoney(pf.amount, window.saso.money_format) + "</span>"
        }
        tiers_copy[i].price_amount = price.amount;
        tiers_copy[i].price_flat = pf;
        tiers_copy[i].price_line = JSON.parse(JSON.stringify(pf));
        tiers_copy[i].price_line.amount = tiers_copy[i].price_line.amount * tiers_copy[i].quantity;
        tiers_copy[i].price_line.title = '<span class="saso-price">' + sasoShopifyformatMoney(tiers_copy[i].price_line.amount, window.saso.money_format) + "</span>"
    }
...
function sasoVolumesAddToCart(ev) {
    ...
    window.sasoc.$first_add_to_cart_el.after("<input type='hidden' name='quantity' value='" + jQuery(this).data("quantity") + "'>");
    ...
    sasoRemovePreviousFreeGifts(function() {
        jQuery.ajax({
            ...
            success: function(res) {
                ...
                sasoShowCrossSellClosed();
                var a = window.saso.action_crosssell_popup_action;
                if (window.sasoc.$cart_offers && window.sasoc.$cart_offers.length || typeof a.price_discount == "object" && a.price_discount && typeof a.price_discount.amount == "number" || typeof a.price_bundle == "object" && a.price_bundle) {
                    setTimeout(function() {
                        window.location.href = "/cart"
                    }, 100)
                } else {
                    jQuery(".saso-product-container[data-product-id=" + product_id + "]").fadeOut("slow", function() {
                        jQuery(".saso-product-container[data-product-id=" + product_id + "]").remove();
...
function sasoRemovePreviousFreeGifts(callback) {
    ...
    if (typeof window.saso.discounts.multiple_offers_free_gift != "string" || window.saso.discounts.multiple_offers_free_gift != "one") {
        return callback()
    }
    if (typeof window.saso.ignore_previous_free_gifts == "boolean" && window.saso.ignore_previous_free_gifts) {
        return callback()
    }
...
function sasoGetHost() {
    var sasoHost = ("saso-www" + Math.round(Math.random() * 10)).replace("www0", "www2").replace("www11", "www9").replace("www10", "www8");
    if (typeof sasoGetParameterByName("d") == "string" && sasoGetParameterByName("d") == "y") {
        sasoHost = "saso-www"
    }
...
function sasoStart() {
    ...
    if (window.sasoDocCookies.getItem("saso_link_code")) {
        window.saso.only_special_link_code = window.sasoDocCookies.getItem("saso_link_code")
    }
    if (window.sasoDocCookies.getItem("saso_discount_code")) {
        window.saso.discount_code_cart = window.sasoDocCookies.getItem("saso_discount_code").toLowerCase().trim()
    }
    if (window.sasoDocCookies.getItem("saso_generated_discount_code")) {
        window.saso.saso_generated_discount_code = window.sasoDocCookies.getItem("saso_generated_discount_code");
    ...
    setTimeout(function() {
        ...
        var selectors = ["input[name='add']:not(.saso-ignore)", "button[name='add']:not(.saso-ignore)", "#add-to-cart", "#AddToCartText", "#AddToCart", "#addToCart", "button[value='Add to cart']", "button[value='Add to Cart']", "input[value='Add to cart']", "input[value='Add to Cart']", "button.addtocart"];
        ...
        selectors.forEach(function(selector) {
            ...
            if (window.saso_config.crosssell_popup_on_add) {
                jQuery(selector).on("click", function(e) {
                    ...
                    var ids = window.saso.action_crosssell_el.$el.parents("form:first").find("[name='id']");
                    if (ids.length == 1) {
        ...
        if ((window.saso.page_type == "collection" || window.saso.page_type == "product") && found_selectors == 0) {}
        if (jQuery(".saso-summary").length == 0 && jQuery(".saso-cart-total").length > 0) {
            jQuery('<div class="saso-summary"></div>').insertBefore(".saso-cart-total")
        }
        if (window.saso.page_type == "product" && (jQuery(".saso-volumes").length == 0 || jQuery(".saso-bundle").length == 0)) {
        ...
        jQuery("body").on("click", ".saso-cross-sell-popup .saso-add-to-cart:not(.saso-bundle-add-to-cart)", sasoUpsellAddToCart);
        jQuery("body").on("click", ".saso-bundle-add-to-cart", sasoBundleAddToCart);
        jQuery("body").on("click", ".saso-volume-discount-tiers .add-to-cart", sasoVolumesAddToCart);
        jQuery("body").on("click", ".saso-cross-sell-popup .add-to-cart", sasoUpsellAddToCart);
        jQuery("body").on("click", "#saso-use-discount-code-cart-apply", sasoUseDiscountCodeCartApply);
        jQuery("body").on("click", ".saso-use-discount-code-cart-apply", sasoUseDiscountCodeCartApply);
        jQuery("body").on("change", "#saso-use-discount-code-instead-check", sasoUseDiscountCodeInsteadChange);
        ...
        jQuery("body").on("change", ".saso-variants", function(ev) {
            ...
            if (typeof img == "string" && img.length) {
                jQuery(ev.target).closest(".saso-product-container").find("img").attr("src", img)
            }
            var price = sasoShopifyformatMoney(jQuery(this).find(":selected").data("price"), window.saso.money_format);
            var compare_at_price = sasoShopifyformatMoney(jQuery(this).find(":selected").data("compare-at-price"), window.saso.money_format);
            jQuery(ev.target).closest(".saso-product-container").find(".saso-price").html(price);
            jQuery(ev.target).closest(".saso-product-container").find(".saso-was-price").html(compare_at_price);
            sasoCurrency()
        });
        if (window.saso.page_type == "cart") {
        ...
        jQuery("body").on("click", ".saso-cross-sell-popup .saso-close", function(ev) {
            ev.preventDefault();
    ...
    if (window.saso.page_type == "product") {
        if (typeof window.saso_extras.product.variants == "object") {
            window.saso_extras.current_variant_id = window.saso_extras.product.variants[0].id
        }
        if (sasoGetParameterByName("variant") != "") {
            window.saso_extras.current_variant_id = parseFloat(sasoGetParameterByName("variant"))
        }
        if (typeof window.saso_extras.current_variant_id == "number" && typeof window.saso_extras.product.variants == "object") {
            var variant = window.saso_extras.product.variants.filter(function(v) {
                return v.id == window.saso_extras.current_variant_id
            });
            if (variant.length == 1) {
    ...
    if (window.saso.page_type == "cart") {
        if (window.sasoDocCookies.getItem("saso_use_discount_code_instead")) {
            ...
            if (document.getElementById("saso-use-discount-code-instead-check")) {
                document.getElementById("saso-use-discount-code-instead-check").checked = true
            }
            return
        }
    }
    ...
    setTimeout(function() {
        if (jQuery("button[onclick=\"window.location='/checkout'\"]").length) {
            console.log("WARNING: buttons with onclick=\"window.location='/checkout'\" will skip Ultimate Special Offers. Replace with /cart instead.")
        }
    }, 1e3)
}
sasoStart();
